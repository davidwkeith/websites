...
/**
 * Do Not Track Compliance Policy (Version 1.0).
 * Standard EFF DNT policy document with the site URL injected.
 *
 * @see https://www.eff.org/dnt-policy
 */
export function registerDntPolicy(eleventyConfig: any, options: SharedPluginOptions): void {
  const policyUrl = new URL("/.well-known/dnt-policy.txt", options.url).href;

  eleventyConfig.addTemplate(
    "shared-dnt-policy.11ty.js",
    {
      render() {
        return `Do Not Track Compliance Policy
...
be posted via HTTPS at ${policyUrl}
...
opinions, interests or activities of the user.`;
    ...
    {
      permalink: "/.well-known/dnt-policy.txt",
